...
const OperatorManage = (param) => {
  const classes = useStyles();
  const [operOpen, setOperOpen] = useState(false);
  const [operNo, setOperNo] = useState(false);
  ...
  // 검색 버튼 
  const handleLoad = () => {
    param.consultantSearchLoad('', condition, query.value);
    setPage(0);
  };
  ...
  function createData(num, name, tel, gender, id, email,  enrolldate, cls, detailbtn) {
    return { num, name, tel, gender, id, email, enrolldate, cls, detailbtn};
  }
  
  const rows = [];

  if(param.consultant){
    for(let i=0;i<param.consultant.data.length ;i++){
      const list = param.consultant.data[i];

      rows.push(createData(
        i+1,list.employ_nm,list.employ_phone,
        list.employ_gender == "FEMALE" ? '여' : '남',
        list.employ_id,
        list.employ_email,
        list.operator_reg_date_time.substr(0,4)+"."+list.operator_reg_date_time.substr(4,2)+"."+list.operator_reg_date_time.substr(6,2)+" "+list.operator_reg_date_time.substr(8,2)+":"+list.operator_reg_date_time.substr(10,2),
        list.employ_member_gb == "OPERATOR" ? '상담원' : '관리자',
        list.operator_no,
        ),);
    };
  }
  ...
  return (
    <div className={classes.root}>
        <div style={{fontSize:22}}>직원 관리</div>
      ...
      <Grid className={classes.gridContainer}>
        ...
        <Grid  container item xs justify="flex-end" alignItems='center'>
          <Grid>
            <div className={classes.enrollIcon} onClick={handleOperOpen}>직원 등록</div>
          </Grid>
        </Grid>
      ...
      />
        ...
        <OperatorModal operNo={operNo} setOperNo={setOperNo} operOpen={operOpen} setOperOpen={setOperOpen}/>
    </div>
  );

};

OperatorManage.getInitialProps = async props => {
...
const mapStateToProps = state => {
  return {
    consultant: state.consultant.data,
  };
};

const mapDispatchToProps = dispatch => ({
  consultantLoad: (num) => dispatch(loadConsultant(num)),
  consultantSearchLoad: (num, condition, query) => dispatch(loadConsultant(num, condition, query)),
});
